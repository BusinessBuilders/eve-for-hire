...
- Task: 
    ...
    - **Report Result**: Call `npx tsx scripts/swarm-tools/report-build-result.ts <orderId> <state> <siteUrl> [error]` to notify the backend.
...
- **Report**: `npx tsx scripts/swarm-tools/report-build-result.ts <orderId> <state> <siteUrl> [error]`
